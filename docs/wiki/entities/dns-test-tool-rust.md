...
| `src-tauri/src/dns/mod.rs` | 测试引擎 | hickory UDP 直连，测试域名 `www.baidu.com.`（尾点 FQDN），3 次采样均值；等级阈值 极佳≤49 / 良好≤99 / 一般≤199 / 较慢≥200（ms）；`sort_results`：成功在前、延迟升序；`TestResult` 的统计字段带 `#[serde(default)]`（只有 `server` 必填，容错前端的极简回传） |
...
- 2026-09-20：修复「生成报告」报 `missing field jitterMs`——前端改为透传 `testDns` 结果、`TestResult` 统计字段加 `#[serde(default)]`，并补 `diagnose_with_results` IPC 回归测试（详见 [diagnose 结果字段契约](../queries/diagnose-results-field-contract.md)）。
- 2026-09-20：修复两个 GitHub Actions 工作流引用的 action 名（`dtolnay/rust-action` 不存在），发布链改为 `tauri-action`，见 [GitHub Actions 工作流](github-actions-workflows.md)。
