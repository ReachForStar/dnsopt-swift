...
1. 左上角选择要设置的**网络接口**（显示连接状态，点“刷新”重新枚举）
2. “DNS服务器配置”区每行输入一个 DNS 地址（`#` 开头为注释），或点“加载常用”带入 23 个常用模板
3. 点“开始测试”：结果按延迟排序并标注等级，自动选中最快两个作为首选/辅助
4. “DNS设置”区可手动调整首选/辅助 → 点“应用选定DNS”（弹 UAC 确认）
5. 随时可用“恢复自动DNS”切回 DHCP，“刷新DNS缓存”清理本地缓存
...
- **v3.0.0**（2026-09）：Rust + Tauri 2 完整重写；hickory 直连 UDP 替代 dig 捆绑；新增 CLI 模式与 IPC 契约测试
